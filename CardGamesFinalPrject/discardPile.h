...
#endif
template<class T>
inline DiscardPile<T>::DiscardPile()
{
	qty = 0;
	max = 1024;
	topPtr = NULL;
...
template<class T>
inline DiscardPile<T>::DiscardPile(int maxSize)
{
	qty = 0;
	max = maxSize;
	topPtr = NULL;
...
template<class T>
inline bool DiscardPile<T>::push(T c)
{
	s_node<T>* temp;
	if (isFull())
	{
...
template<class T>
inline bool DiscardPile<T>::pop(T& c)
{
	s_node<T>* temp;
	if (isEmpty())
	{
...
template<class T>
inline bool DiscardPile<T>::peek(T& c)const
{
	if (isEmpty())
	{
		return false;
...
template<class T>
inline s_node<T>* DiscardPile<T>::getTop()
{
	return this->topPtr;
}
template<class T>
inline bool DiscardPile<T>::isEmpty()const
{
	if (topPtr == NULL)
	{
		return true;
...
template<class T>
inline bool DiscardPile<T>::isFull()const
{
	if (qty >= max)
	{
		return true;
...
template<class T>
inline bool DiscardPile<T>::makeEmpty()
{
	s_node<T>* temp;
	if (isEmpty())
	{
...
template<class T>
inline int DiscardPile<T>::getQty()const
{
	return qty;
}

template<class T>
inline void DiscardPile<T>::displayStack() const
{
		s_node<T>* temp;
		temp = topPtr;
		while (temp != NULL)
